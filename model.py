...
config = GPTConfig(
    block_size=16,   # Use a small block size for the test.
    vocab_size=100,  # A small vocabulary size.
    n_layer=2,       # A couple of layers.
...
# Instantiate the GPT model with the test configuration.
model = GPT(config)

# Create a dummy input token sequence with batch size = 2 and sequence length = 10.
input_tokens = torch.randint(0, config.vocab_size, (2, 10))
print("Input tokens:", input_tokens)

# Run the forward pass in training mode (compute logits and loss).
...
# Using the same dummy input tokens as before.
# Generate 5 new tokens using a specified temperature and top_k filtering.
generated_tokens = model.generate(input_tokens, max_new_tokens=5, temperature=0.8, top_k=5)
